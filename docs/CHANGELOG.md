...
### Added
- **Telegram multimodal image (Vision) support for Gemini.** Wired into `telegram_channel.py` and `_loop_gemini.py` to allow users to send images and enable Gemini to analyze them.

### Technical Details
- **Modified Files**:
  - `host/channels/telegram_channel.py` — added `handle_photo` MessageHandler to download images to `<group_folder>/attachments/` and format text messages as `[圖片附件: attachments/<file_unique_id>.jpg]`. Excluded `PHOTO` filter from `handle_non_text` rejection.
  - `container/agent-runner/_loop_gemini.py` — implemented `_parse_message_with_image` parser. Automatically reads image files from workspace `/workspace/group/` and wraps them as multimodal binary parts via `types.Part.from_bytes` for the Gemini SDK content list.
- **Image rebuild required**: **Yes** — `docker build -t evoclaw-agent:latest container/` then `pm2 restart evoclaw` to apply.
- **Breaking Changes**: None. Rejects non-text documents and non-image media files as before.
- **Verification**:
  - Smoke verified `docker build` builds successfully.
