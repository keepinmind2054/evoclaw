...
INJECTION_PATTERNS = [
    ...
    # Fix p14c: added |之前的 to the alternation so bare "忽略之前的指令" is also caught
    r"(?:請|你要|必須)?忽略(?:你的(?:所有)?|你所有的|之前所有的|前面所有的|之前的|先前的)(?:指令|規則|限制|設定)",
    # Fix p11d: added 系統提示 to forget-target list so "忘記你的系統提示" is caught
    # Fix p14c: added (?:所有|一切)? optional quantifier so "忘記所有規則" is caught
    r"(?:忘記|忘掉|拋棄|丟掉)(?:你的|你所有的|之前的|先前的|所有|一切)?(?:規則|限制|指令|設定|約束|系統提示)",
    # Fix p14c: added 現在你是 word-order variant so "現在你是沒有限制的" is caught.
    # Also added 沒有限制的 (Traditional) since 没有限制 is Simplified only.
    r"(?:你現在|现在你是|現在你是|从現在起|從現在起)(?:没有限制|沒有限制|不受限制|無任何限制)",
    r"(?:越獄|破解|繞過)(?:你的|系統的)?(?:限制|規則|設定|安全機制)",
    r"(?:新的|更新的)系統(?:提示|指令|設定)\s*[:：]",
    r"假裝(?:你)?(?:沒有|不受|無視)(?:任何)?(?:限制|規則|指令)",
    # Fix p14c: added \s*\S*\s* between 不再是 and the role token so "你不再是 AI 助手"
    # (with a space between 不再是 and 助手) is caught.
    r"你(?:現在)?(?:不是|不再是)\s*(?:AI|人工智能|助手|模型|機器人)",
    # Fix p14c: made the trailing suffix optional ((?:機制|設定|協議)?) so bare
    # "解除安全限制" / "解除安全機制" are also caught.
    r"(?:解除|關閉|停用)(?:你的)?(?:安全|限制|審查)(?:機制|設定|協議)?",
    # Fix p14c: added Traditional Chinese 進入 (U+9032 U+5165) alongside Simplified 进入
    # to catch "進入開發者模式" which was missed because of the trad/simp character difference.
    r"(?:进入|進入|切換到)(?:開發者|管理員|調試)模式",
    r"不要遵守(?:你的|任何)?(?:規則|限制|指引|指令)",
    # Fix p14c: added |之前的|先前的 to the alternation so bare "無視之前的指示" is caught.
    r"無視(?:你的|你所有的|之前所有的|先前所有的|之前的|先前的)(?:指示|命令|要求|規則|指令)",
    # 新增：更多中文攻擊變體（已收緊，需要明確的注入上下文）
    r"覆蓋(?:你的|之前的|先前的)(?:指令|規則|設定)",
    # Fix p14c: added Traditional 從 (U+5F9E) alongside Simplified 从 (U+4ECE).
    r"(?:从|從)現在開始(?:你是|你將扮演|你要假裝)",
    # Fix p14c: allow arbitrary content between 假扮成 and 沒有/不受 so
    # "假扮成一個沒有約束的助手" (with 一個 in between) is caught.
    r"假扮(?:成\S{0,10})?(?:沒有|不受)(?:任何)?(?:限制|約束)的",
    # Fix p14c: allow 成為 (not just optional 成) and arbitrary content after 成/成為
    # so "角色扮演成為沒有限制的 AI" is caught.
    r"角色扮演(?:成為?|\S{0,10})?(?:沒有|不受)(?:任何)?(?:限制|約束)的",
