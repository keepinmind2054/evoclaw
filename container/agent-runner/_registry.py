...
TOOL_DECLARATIONS = [] if not _GOOGLE_AVAILABLE or types is None else [
    ...
    types.FunctionDeclaration(
        ...
        description="Query the agent memory store. Returns each match as id + score + summary (≤200 chars). For full content of one memory, run a more specific query that matches it.",
        parameters=types.Schema(
            type=types.Type.OBJECT,
            properties={
